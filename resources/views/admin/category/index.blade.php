                ...
                <div class="modal fade" id="view" tabindex="-1" aria-labelledby="exampleModalLabel"
                    ...
                    <div class="modal-dialog">
                        <div class="modal-content">
                            ...
                            <div class="modal-body render_category_show">
        ...
        <div class="col-lg-5">
            <div class="card custom-card overflow-hidden border px-2">
                {{-- @if (isset($data['category']))
                    <form action="{{ route($base_route . 'update', $data['category']->id) }}" method="POST"
                ...
                <form action="{{ route($base_route . 'store') }}" method="POST" enctype="multipart/form-data"
                    ...
                    <div class="card-body">
                        ...
                        <h6 class="card-title">{!! isset($data['category']) ? '<i class="ri-edit-2-fill"></i>' : '<i class="ri-add-large-fill"></i>' !!}
                            {{ isset($data['category']) ? 'Update' : 'Create' }}
                            {{ $panel ?? '' }}(Page)</h6>
                        <hr />
                        ...
                        <div class="tab-content" id="profile-tabs">
                                ...
                                <div class="col-md-12 mb-3">
                                    <div class="form-floating">
                                        <input type="text" name="title"
                                            value="{{ isset($data['category']->title) ? $data['category']->title : old('title') }}"
                                ...
                                <div class="col-md-12 mb-3">
                                    <div class="form-floating">
                                        <input type="text" name="sub_title"
                                            value="{{ isset($data['category']->sub_title) ? $data['category']->sub_title : old('sub_title') }}"
                                    ...
                                    <div class="form-group">
                                        <div class="image dropify-wrapper">
                                            @if (!empty($data['category']->image))
                                                <img src="{{ asset($img_path . $data['category']->image) }}"
                            ...
                            <div class="tab-pane p-0 border-0 py-3" id="social-media-pane" role="tabpanel"
                                ...
                                <textarea name="description" class="form-control summernote" id="editor3">{{ isset($data['category']->description) ? $data['category']->description : old('description') }}</textarea>
                            </div>
                                    ...
                                    <div class="form-group">
                                        <div class="image dropify-wrapper">
                                            @if (!empty($data['category']->banner))
                                                <img src="{{ asset($img_path . $data['category']->banner) }}"
                            ...
                            <div class="tab-pane p-0 border-0" id="reset-password-pane" role="tabpanel"
                                ...
                                <div class="col-md-12 mb-3">
                                    <div class="form-floating">
                                        <input type="text" name="seo_title"
                                            value="{{ isset($data['category']) ? $data['category']->seo_title : '' }}"
                                ...
                                <div class="col-md-12 mb-3">
                                    <div class="form-floating">
                                        <input type="text" name="seo_keyword"
                                            value="{{ isset($data['category']) ? $data['category']->seo_keyword : '' }}"
                                ...
                                <div class="col-md-12 mb-3">
                                    <div class="form-floating">
                                        <textarea name="seo_description" value="" class="form-control" placeholder="Seo Description">{{ isset($data['category']) ? $data['category']->seo_description : '' }}</textarea>
                            ...
                            <button type="submit" data-button="page"
                                class="formButton  btn btn-{{ isset($data['category']) ? 'danger' : 'primary' }} text-center mx-auto btn-w-md d-flex align-items-center justify-content-center btn-wave waves-light text-nowrap waves-effect waves-light"
                                data-bs-toggle="modal" data-bs-target="#create-folder">
                                {!! isset($data['category']) ? '<i class="ri-edit-2-fill p-1"></i>' : '<i class="ri-add-large-fill p-1"></i>' !!}
                                {{ isset($data['category']) ? 'Update' : 'Create' }}
        ...
        <div class="modal fade" id="edit" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered modal-lg">
                <div class="modal-content">
                    ...
                    <div class="modal-body render_category_edit">
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '.view-category', function() {
                ...
                let url = "{{ url('/admin/category') }}/" + id;

                $.ajax({
                    url: url,
                    type: "GET",
                    success: function(resp) {
                        $('.render_category_show').html(resp);
                    },
                    error: function(xhr) {
            ...
            $(document).on('click', '.edit-category', function() {
                ...
                $.ajax({
                    ...
                    type: "GET",
                    success: function(resp) {
                        $('.render_category_edit').html(resp);
                        initializeCkEditor();
                        // $('#edit').modal('show');
                    },
                    error: function(xhr) {
