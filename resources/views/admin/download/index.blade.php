                ...
                <div class="modal fade" id="view" tabindex="-1" aria-labelledby="exampleModalLabel"
                    ...
                    <div class="modal-dialog">
                        <div class="modal-content">
                            ...
                            <div class="modal-body render_download_show">
            ...
            <div class="card custom-card overflow-hidden border px-2">
                ...
                <form action="{{ route($base_route . 'store') }}" method="POST" enctype="multipart/form-data"
                    ...
                    <div class="card-body">
                        ...
                        <h6 class="card-title">{!! isset($data['download']) ? '<i class="ri-edit-2-fill"></i>' : '<i class="ri-add-large-fill"></i>' !!}
                            {{ isset($data['download']) ? 'Update' : 'Create' }}
                            {{ $panel ?? '' }}(Page)</h6>
                        <hr />
                        ...
                        <div class="tab-content" id="profile-tabs">
                                ...
                                <div class="col-md-12 mb-3">
                                    <div class="form-floating">
                                        <input type="text" name="title"
                                            value="{{ isset($data['download']->title) ? $data['download']->title : old('title') }}"
                                ...
                                <div class="col-md-12 mb-3">
                                    <div class="form-floating">
                                        <input type="text" name="sub_title"
                                            value="{{ isset($data['download']->sub_title) ? $data['download']->sub_title : old('sub_title') }}"
                                    ...
                                    <div class="form-group">
                                        <div class="image dropify-wrapper">
                                            @if (!empty($data['download']->image))
                                                <img src="{{ asset($img_path . $data['download']->image) }}"
                            ...
                            <div class="tab-pane p-0 border-0 py-3" id="social-media-pane" role="tabpanel"
                                ...
                                <textarea name="description" class="form-control summernote" id="editor3">{{ isset($data['download']->description) ? $data['download']->description : old('description') }}</textarea>
                            </div>
                            ...
                            <div class="tab-pane p-0 border-0" id="banner-pane" role="tabpanel"
                                ...
                                <div class="col-md-12 mb-3 image">
                                    ...
                                    <div class="form-group">
                                        <div class="image dropify-wrapper">
                                            @if (!empty($data['download']->banner))
                                                <img src="{{ asset($img_path . $data['download']->banner) }}"
                            ...
                            <button type="submit" data-button="page"
                                class="formButton  btn btn-{{ isset($data['download']) ? 'danger' : 'primary' }} text-center mx-auto btn-w-md d-flex align-items-center justify-content-center btn-wave waves-light text-nowrap waves-effect waves-light"
                                data-bs-toggle="modal" data-bs-target="#create-folder">
                                {!! isset($data['download']) ? '<i class="ri-edit-2-fill p-1"></i>' : '<i class="ri-add-large-fill p-1"></i>' !!}
                                {{ isset($data['download']) ? 'Update' : 'Create' }}
        ...
        <div class="modal fade" id="edit" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered modal-lg">
                <div class="modal-content">
                    ...
                    <div class="modal-body render_download_edit">
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '.status_show_hide', function() {
                ...
                $.ajax({
                    url: "{{ route('admin.download.status') }}",
                    data: {
                        id: id
                    },
            ...
            $(document).on('click', '.view-download', function() {
                ...
                // let url = "{{ route('admin.download.show', ':id') }}"; 
                // url = url.replace(':id', id);
                let url = "{{ url('/admin/download') }}/" + id;

                $.ajax({
                    url: url,
                    type: "GET",
                    success: function(resp) {
                        $('.render_download_show').html(resp);
                    },
                    error: function(xhr) {
            ...
            $(document).on('click', '.edit-download', function() {
                let id = $(this).data('id');

                let url = "{{ route('admin.download.edit', ':id') }}";
                url = url.replace(':id', id);
                // let url = "{{ url('/admin/download') }}/" + id;
                ...
                $.ajax({
                    ...
                    type: "GET",
                    success: function(resp) {
                        $('.render_download_edit').html(resp);
                        initializeCkEditor();
                        // $('#edit').modal('show');
                    },
                    error: function(xhr) {
